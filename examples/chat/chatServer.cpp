...
private:
    void onConnection(const TcpConnectionPtr& conn){
        ...
        //调用onConnection时，TcpConnection一定一定存活（由TcpConnection成员函数调用）
        //实际上不改变TcpConnection生命期，TcpConnection新建，建立连接后，connections_.insert(conn)
        //使TcpConnectionPtr增加一个引用计数，TcpConnection断开连接，准备销毁之前，
        //connections_.erase(conn)，使TcpConnectionPtr减少一个引用计数
        if (conn->connected()){
